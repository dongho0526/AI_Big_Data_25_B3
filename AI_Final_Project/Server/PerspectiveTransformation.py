...
class PerspectiveTransformation:
    ...
    def __init__(self):
        ...
        # Adjust points to 320x240 resolution
        self.src = np.float32([
            (550 * 320 / 1280, 460 * 240 / 720),  # top-left
            (150 * 320 / 1280, 720 * 240 / 720),  # bottom-left
            (1200 * 320 / 1280, 720 * 240 / 720), # bottom-right
            (770 * 320 / 1280, 460 * 240 / 720)   # top-right
        ])
        self.dst = np.float32([
            (100 * 320 / 1280, 0 * 240 / 720),    # top-left
            (100 * 320 / 1280, 720 * 240 / 720),  # bottom-left
            (1100 * 320 / 1280, 720 * 240 / 720), # bottom-right
            (1100 * 320 / 1280, 0 * 240 / 720)    # top-right
        ])
        self.M = cv2.getPerspectiveTransform(self.src, self.dst)
        self.M_inv = cv2.getPerspectiveTransform(self.dst, self.src)

    def forward(self, img, img_size=(320, 240), flags=cv2.INTER_LINEAR):
    ...
    def backward(self, img, img_size=(320, 240), flags=cv2.INTER_LINEAR):
